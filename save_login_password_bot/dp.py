...
from save_login_password_bot.password_bot import dp, bot
...
async def delete_accounts(message):
    pass
    # global base, cur
    # print(login, password)
...
async def sql_read_info(message):
    ...
    try:
        ...
        await bot.send_message(message.from_user.id, 'Добавленых аккаунтов нет')


async def delete_acount_db(message, login):
    try:
        count = 0
