...
from save_login_password_bot.dp import *
...
async def start_choice(message: types.Message, state: FSMContext):
    if message.text=='добавить аккаунт':
        ...
        await bot.send_message(message.from_user.id, 'введите аккаунт ,который ты хочешь удалить',
...
async def delete_acc(message:types.Message,state:FSMContext):
    await delete_acount_db(message=message, login=message.text)
    await state.finish()
...
async def start_mes(message: types.Message, state: FSMContext):
    if message.from_user.id == admin_id:
        ...
        await bot.send_message(message.from_user.id, 'не пытайтесь взломать моего бота')
